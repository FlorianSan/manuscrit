...
{
    \color{blue}
    À partir de la modélisation \eqref{eq:udwadia} et de l'expression des contraintes \eqref{eq:contrainteAB}, il est possible de généré les commande à appliquer pour suivre une référence.

    Nous définissons donc une trajectoire paramétrée par le temps : 
    \begin{align}
        x_{t}(t) = [p_{t},v_{t}, a_{t}]^\top :=[p_x,p_y,p_z,v_x,v_y,v_z,a_x,a_y,a_z]^\top
    \end{align}

    Nous allons scinder l'architecture de commande pour avoir un ensemble identifiable comme du guidage et un autre pour la stabilisation. Ainsi, le module de guidage utilise les informations de la trajectoire $x_{t}(t)$ et la connaissance de la position $p$ et la vitesse $v$ du drone pour générer une commande de force orienté dans l'espace $F_{c}$. Cette force sera l'entrée du module de stabilisation avec l'orientation $q$ et la vitesse angulaire $\omega$ du drone.
    Étant donné que les drones sont sous actionné, il n'est pas possible de généré directement la force $F_{c}$ avec les actionneurs. Il est nécessaire de modifier l'orientation du drone $q$ pour aligner la force généré par ce dernier sur la force désiré $F_{c}$.

    Nous allons commencer par décrire le module de guidage. Ce dernier se s'intéresse qu'à la dynamique transactionnelle du drone. Nous avons donc : 
    \begin{align}
        F_{c} = M_{p}*\left((a_{t} -k_{d}(v-v_{t}) - k_{p}(p-p_{t})) - M_{p}^{-1}\begin{bmatrix}
            0\\0\\g
        \end{bmatrix}\right)
    \end{align}

    Le module suivant est la stabilisation :
    \begin{align}
        u = B^{\dag}
    \end{align}
...
\todo{Vols expérimentaux unifiés}

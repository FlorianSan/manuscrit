...
Dans les sections suivantes, nous notons la marge de module d'une matrice de transfert $s \mapsto T_{v \rightarrow z}$ comm $\Delta_m(T_{v \rightarrow z}) = \min\limits_{\omega\in R} \sigma_{\min}(T_{v \rightarrow z}(j\omega))$.
\todo{J'utilise cette notation dans la section suivante}
...
\end{align*} 
\todo{deplacer les transfert vers le chapitre 4}
...
\todo{ajouter le point qui augmente W}
...
\begin{figure}[ht!]
    ...
    \label{fig:statpos}
\end{figure}
\todo{ajouter une legende dans le caption}

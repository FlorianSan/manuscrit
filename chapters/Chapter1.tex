...
\section{Micro drones convertibles}
   ...
   Nous observons sur la Table \ref{tab:architecture} qu'un drone convertible possède un domaine de vol bien plus important qu'un drone à voilure fixe, lequel ne sera pas en mesure de voler à très basse vitesse, et qu'un quadrirotor, dont l'autonomie va être limitée par sa consommation. Toutefois, les drones convertibles sont nettement moins efficace qu'une voilure fixe car les convertibles sont plus lourds en raison des mécanismes nécessaire à la conversion et des redondances nécessaire pour lutter contre les pannes qui ont un impact désastreux sur les convertibles. Il faut aussi noter que les voilures fixes ont une consommation inférieure pour les mêmes raisons qu'évoquées précédemment. Les drones convertibles ont une consommation variable qui dépends de l'architecture (liée à la masse ajoutée) et du temps passés en vol stationnaire et en vol d'avancement.
...
L'approche "Diviser et Conquérir" de la Figure \ref{fig:schemaGainSche} (a) permet de sélectionner la loi de contrôle approprié pour chaque phase de vol, de manière discrète, de sorte qu'un seul contrôleur ne fonctionne à la fois. Un exemple d'un contrôle séquencé hybride de \textit{tailsitters} est donnée dans \cite{Casau2011}.
...
\label{sec:methodo}
{\color{blue}
    Il existe plusieurs méthodes pour obtenir les gains des contrôleurs évoqués précédemment (section \ref{sec:archcmd}). L'ensemble de méthode commence par la plus naïve avec un réglage des gains d'un PID par essais erreur et va jusqu'à des méthodes complexes d'optimisation (optimisation non lisse pour synthèse multiobjectif) en passant par la résolution de l'équation algébrique de Riccati pour obtenir la matrice de gain d'un contrôleur LQR (section \ref{sec:LQR}). Nous nous attarderons ici sur deux méthodes, le réglage des gains d'un PID par une méthode de placement de pole et une synthèse basée sur de critères $H_{\infty}$.


\subsection*{Obtention des gains d'un PID par une méthode de placement de pole \cite{WANG2008PID}}
L'obtention des gains proportionnel, intégral et dérivatif d'un contrôleur PID très largement utilisé dans l'industrie est souvent réalisé à l'aide d'un réglage manuel des valeurs. Toutefois, à l'aide d'un modèle, il est possible d'obtenir les gains qui assurent des critères de performance (rapidité et dépassement). Cette méthode a été proposée par \cite{WANG2008PID}.
...
\end{align}
Cette équation complexe peut être résolu en exprimant $K_{i}$ et $K_{d}$ en fonction de $K_{p}$ :
...
{\color{blue}
Bien que les modèles proposés soient de plus en plus précis (section \ref{sec:modelisation}), que les simulations soient de plus en plus réaliste et que par conséquent, nous soyons en mesure de proposer des lois de commande de plus en plus élaboré (section \ref{sec:archcmd} et \ref{sec:methodo}), tout ce travail n'a pour validation finale que l'expérimentation. Ce travail nécessite  le montage d'une plateforme composé de plusieurs composants, l'identification des coefficients des modèles, le transfert des lois de commande dans un autopilote et la mise en place de vol expérimentaux.
}

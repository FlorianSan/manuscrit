...
{\color{magenta}
    Des points équilibres obtenus dans la section \ref{sec:eqpoint} et notamment de l'Algorithme~\ref{alg:eq}, il semble cohérent de poursuivre notre travail en obtenant les dynamiques linéarisés autours des points d'équilibre. Ces résultats permettront, par exemple, de concevoir des lois de commande linéaire basée modèle. De nombreux outils existent pour le contrôle des modèles linéaires. Tout l'intérêt réside dans le fait d'avoir une modèle linéarisé autour d'un point d'équilibre assez consistant. Nous pouvons définir la consistance d'une linéarisation comme la précision du modèle linéaire vis-à-vis du modèle non-linéaire. Nous sommes intéressés par la meilleure prise en compte des phénomènes non-linéaire tel que les actions aérodynamiques.

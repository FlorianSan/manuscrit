...
Ainsi, citon en exemples le \textit{tailsitter} à double rotors appelé «~T-Wing~» \cite{Stone2002PreliminaryDO, TWing2008}, le tail-sitter appelé «~MavIon~» \cite{oatao14575}, ou le «~JLion~» et le «~KH-Lion~» \cite{8003167}. Ces drones partagent une architecture similaire basée sur une aile supportant deux moteurs sur le bord d'attaque et soufflant deux élevons situés sur le bord de fuite. Cette architecture offre une plus grande robustesse que les \textit{tiltrotors}, composés de pièces mobiles (ce qui les rend plus fragiles) et d'un actionneur puissant pour faire tourner l'ensemble moteur-hélice.
...
\todo{Rejet de perturbation et métrique de maintien de position }

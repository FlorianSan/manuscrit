...
De nombreuse maquette réelle ont été assembler dans le but de réaliser des vols expérimentaux. Citons en exemples le \textit{tailsitter} à doubles rotors appelé «~T-Wing~» \cite{Stone2002PreliminaryDO, TWing2008}, le \textit{tailsitter} appelé «~MavIon~» \cite{oatao14575}, ou le «~JLion~» et le «~KH-Lion~» \cite{8003167}. Ces trois maquettes sont illustrées sur la figure \ref{fig:maquettetailsitter}.
...
Comme expliqué précédemment, lors des phases de décollage et d'atterrissage, le drone est vertical (voir figure~\ref{fig:darko_flight}) ce qui engendre une grande sensibilité aux perturbations. Il semble pertinent de ce concentré nos travaux sur l'étude de la robustesse des drones convertible face au vent.
...
Notre travail se concentre sur la recherche d'un contrôleur de vol unifié, pour une architecture de drone fortement non-linéaire et couplée, sur l'intégralité du domaine de vol, en environnement perturbé.

\section*{Plan et contribution}
Notre exposé commencera par une description générale des architectures de drone convertible (Chapitre~\ref{chap:generalites}) avec une description des avantages et des inconvénients ainsi que leur mode de fonctionnement. Une description générale de la modélisation, de l'actionnement et des lois de commande proposée sur les \textit{tailsitters} et les \textit{freewings} ouvrira nos propos sur ces architectures.
...
Les objectifs fixés sont pluriels : 
il s'agira d'étudier le comportement d'un drone \textit{tailsitters} en environnement perturbé, saturations des actionneurs, dynamiques linéarisées, impact des non-linéarités et cycles limites. De proposer des architectures de commande basées modèle pour un drone \textit{tailsitters} permettant d'assurer une robustesse aux perturbations de vent, ouvrant la possibilité de certification de ce type d'architecture. Enfin d'utiliser des capteurs pour mesurer les perturbations en avance de phase pour les rejeter (sonde 5 trous, micro, Pitot, etc.), ce qui implique de développer une nouvelle architecture.
